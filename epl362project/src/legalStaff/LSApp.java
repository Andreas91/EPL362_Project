...
public class LSApp extends JFrame {
	...
	public LSApp() {
		...
		// Appointments Table
		String[] col = { "A/a", "Type", "Date/Time", "Scheduled",
				"Transaction", "Updated" };
		Object[][] data = {};
		...
		btnCShow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (tableClients.getSelectedRowCount() != 1) {
					JOptionPane.showMessageDialog(null,
							"Select a client to show!");
				} else {
					Object o = modelC.getValueAt(tableClients.getSelectedRow(),
							0);
					getClient((int) o);
				}
			}
		...
		btnCCom.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (tableClients.getSelectedRowCount() != 1) {
					JOptionPane.showMessageDialog(null,
							"Select a client to see/add comment(s)!");
				} else {
					Object o = modelC.getValueAt(tableClients.getSelectedRow(),
							0);
					int cid = (int) o;
					if (comm != null && comm.isVisible())
						comm.dispose();
		...
		btnOShow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (tableOpinions.getSelectedRowCount() != 1) {
					JOptionPane.showMessageDialog(null,
							"Select an opinion to show!");
				} else {
					Object o = modelO.getValueAt(
							tableOpinions.getSelectedRow(), 0);
					getOpinion((int) o);
				}
			}
		});
	...
	public void insertApp() {
		...
		for (int i = 0; i < 20; i++)
			model.addRow(new Object[] { i + 1, "type" + i, (i + 1) + "-Apr-14",
					"Yes", "T00" + i, "Yes" });
	}
