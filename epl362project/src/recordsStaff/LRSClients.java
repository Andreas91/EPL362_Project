...
/**
 * The Clients class, gives the records staff access to all clients records
 * Through the interface, the staff can search any client by their id and
 * make changes.
 * @author Andreas Andreou
 * @version 1.0
 */
@SuppressWarnings("serial")
public class LRSClients extends JFrame {
	...
	private String tID;					// Temporary stores Client's ID if edit is cancel
	private boolean tFlag;				// Temporary stores Client's Flag if edit is cancel
	private String tName;				// Temporary stores Client's Name if edit cancel
	private String tSurname;			// Temporary stores Client's Surname if edit is cancel
	
	
	/**
	 * Class constructor. It creates the frame
	 * for the clients window.
	 * @param user Staff's username.
	 */
	public LRSClients(String user) {
		setBounds(550, 100, 682, 525);
		contentPane = new JPanel();
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		setContentPane(contentPane);
		contentPane.setLayout(null);
		this.username=user;
		setTitle("Clients - "+this.username);
		...
		scrollPane.setBounds(21, 154, 630, 144);
		...
		btnCSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				saveClient(Integer.valueOf(tID));
				editClient();
			}
		});
	...
	/**
	 * Fills the table with all the clients.
	 */
	public void insertClients() {
		...
		String str = "SELECT * FROM dbo.CLIENT";
		Object[][] rs = (Object[][]) client.send(str);
		
		// Insert clients to table
		for (int i=1;i<rs.length;i++){
			model.addRow(new Object[] {rs[i][0], rs[i][2], rs[i][3], rs[i][1], rs[i][4]});
		}
	}
	...
	private void editClient() {
		...
		CID.setEditable(!CID.isEditable());
	...
	private void saveClient(int cid) {
		int id = Integer.parseInt(this.CID.getText());
		...
		String str = "UPDATE dbo.CLIENT "+
					 "SET CID='"+id+"', FLAG='"+bFlag+"', FNAME='"+sName+"', LNAME='"+sSurname+"' "+
					 "WHERE CID='"+cid+"';";
